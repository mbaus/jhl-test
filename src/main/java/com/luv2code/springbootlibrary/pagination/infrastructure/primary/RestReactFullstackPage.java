...
public class RestReactFullstackPage<T> {
  ...
  private RestReactFullstackPage(RestReactFullstackPageBuilder<T> builder) {
  ...
  public static <S, T> RestReactFullstackPage<T> from(ReactFullstackPage<S> source, Function<S, T> mapper) {
    Assert.notNull("source", source);
    Assert.notNull("mapper", mapper);

    return new RestReactFullstackPageBuilder<>(source.content().parallelStream().map(mapper).toList())
  ...
  private static class RestReactFullstackPageBuilder<T> {
    ...
    private RestReactFullstackPageBuilder(List<T> content) {
      this.content = content;
    }

    public RestReactFullstackPageBuilder<T> pageSize(int pageSize) {
      this.pageSize = pageSize;

      return this;
    }

    public RestReactFullstackPageBuilder<T> currentPage(int currentPage) {
      this.currentPage = currentPage;

      return this;
    }

    public RestReactFullstackPageBuilder<T> totalElementsCount(long totalElementsCount) {
      this.totalElementsCount = totalElementsCount;

      return this;
    }

    public RestReactFullstackPageBuilder<T> pageCount(int pageCount) {
      this.pageCount = pageCount;

      return this;
    }

    public RestReactFullstackPageBuilder<T> hasPrevious(boolean hasPrevious) {
      this.hasPrevious = hasPrevious;

      return this;
    }

    public RestReactFullstackPageBuilder<T> hasNext(boolean hasNext) {
      this.hasNext = hasNext;

      return this;
    }

    public RestReactFullstackPage<T> build() {
      return new RestReactFullstackPage<>(this);
    }
  }
}
